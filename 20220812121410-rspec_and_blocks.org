...
* Testing base classes
  ...
  =NOTE= if you need to reference ~let~ block values in the dummy class method
  definitions, use `define_method` with a block and cache the ~let~ method call to
  a variable. See example
