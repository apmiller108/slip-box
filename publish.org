...
*** Links and Images
    ...
    - Org-roam uses the ID property for linking notes. To get around this, first I add the notes to
      the list of places where files with IDs live ~org-id-update-id-locations~.
      This is where org will look for files when finding by ID. Then if org
      finds a file by an ID value (~org-id-find~), replace the ~path~ property
      with the filename.
    ...
    #+begin_src elisp +n :results silent
      ...
      (defun my/org-html-link (link contents info)
        ...
        (print (org-id-find (s-replace "id:" "" (org-element-property :path link))))
        (print (seq-filter 'file-regular-p (directory-files "./" t)))
