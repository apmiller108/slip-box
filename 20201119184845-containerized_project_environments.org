...
* The super quick and dirty
  Just run this terminal command followed by the image to build a container
  from. Maps the current host directory to ~/app~ inside the container.

  #+begin_src shell 
  docker run -it --rm -v "$PWD":/app ruby:latest
  #+end_src
